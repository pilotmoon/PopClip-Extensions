...
*This document applies to [PopClip beta](https://pilotmoon.com/popclip/download).
...
|Key|Type|Required?|Description|
|---|----|---------|-----------|
|`Title`|String or Dictionary|Optional|The title for this action. The title is displayed on the action button if there is no icon. For extensions with icons, the title is displayed in the tooltip. If omitted, the `Extension Name` is used as the title.|
...
|`Icon`|String|Optional| See [Icons](#icons). If you omit this field, the `Extension Icon` will be used. If there is no extension icon, the `Title` will be displayed instead. |
