...
|`After`|String|Optional|String to indicate an action PopClip should take *after* performing the main action. One of: `copy`, `cut`, `paste` or `paste-result`, `copy-result`, `copy-selection` or `popclip-appear`. See [Example Script Returning Result](#example-script-returning-result) for usage of `paste-result` and `copy-result`. The `popclip-appear` value makes PopClip invoke itself again (this is used in the Select All extension). The `copy-selection` value makes PopClip copy the original selected text as plain text (this is used in the Swap extension).|
