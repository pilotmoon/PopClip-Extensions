...
|`After`|String|Optional|String to indicate an action PopClip should take *after* performing the main action. One of: `copy`, `cut`, `paste` or `paste-result`, `copy-result`, `paste-or-copy-result` or `popclip-appear`. See [Script Returning Result](#script-returning-result) for usage of `paste-result`, `copy-result` and `paste-or-copy-result`. The `popclip-appear` value makes PopClip invoke itself again (this is used in the Select All extension).|
...
|Key|Behaviour|
|---|---------|
|`copy-result`|PopClip will copy the returned text to the clipboard.|
|`paste-result`|PopClip will paste the returned text, replacing the current selection.|
|`paste-or-copy-result`|PopClip will paste the returned text if paste is available, otherwise it will copy it to the clipboard.|
