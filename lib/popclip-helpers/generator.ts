 ...
 */
export async function * concurrentTransform<T, U> (items: Iterable<T>, transform: (item: T) => Promise<U>): AsyncIterable<U> {
  // de-duplicate inputs
  ...
  await Promise.all(Array.from(deduped, async (item) => {
