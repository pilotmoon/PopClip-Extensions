...
export const action: ActionFunction = async (selection, context, options) => {
  ...
  const response = await bitly.post('v4/shorten', { long_url: selection.data.webUrls[0] }, {
...
export const auth: AuthFunction = async (info, flow) => {
  ...
  const response = await bitly.post('oauth/access_token',
    util.buildQuery({ client_id, client_secret, redirect_uri, code }),
