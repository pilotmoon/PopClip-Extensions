...
const process = (selection, options) => {
  ...
  const custom_settings =
    ...
    options.locale !== defaultLocale[0];
  if (custom_settings) {
    const formatOptions = Object.fromEntries(
      [["timeZone", options.timeZone]].filter(([k, v]) => v !== ""),
    );
...
const buildFormat = (locales = [], custom_options = {}) => {
  ...
  Object.entries(custom_options).forEach(
  ...
  options: [
    {
      ...
      type: "multiple",
      values: [defaultLocale[0], systemSetting[0]].concat(
        Array.from(LOCALES.keys()),
      ),
      valueLabels: [defaultLocale[1], systemSetting[1]].concat(
    ...
    },
    {
      ...
      label: "Time Zone",
      type: "multiple",
      values: [defaultTimezone[0], systemSetting[0]].concat(
