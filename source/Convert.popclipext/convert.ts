...
function formatOutput(num: number, places: number): string {
...
function places(numberString: string): number {
...
function makeRegex(partial: string): RegExp {
  return new RegExp(
    `^\\s*((\\d+(?:[.,]\\d+)?)(\\s*)(?:${partial}))\\s*$`,
    "ui",
  );
...
const conversions = [
  ...
  {
    regex: makeRegex("kilograms?|kilos?|kg"),
    outputUnit: "lb",
    factor: 1 / kgPerLb,
  },
  ...
  // distance
  {
    regex: makeRegex('inches|inch|ins?|"'),
    outputUnit: "cm",
    factor: cmPerInch,
  },
  ...
  {
    regex: makeRegex("centimetres?|centimeters?|cm"),
    outputUnit: '"',
    factor: 1 / cmPerInch,
    space: 0,
  }, // inch sign is never spaced
  {
    regex: makeRegex("millimetres?|millimeters?|mm"),
    outputUnit: '"',
    factor: 1 / mmPerInch,
    space: 0,
  },
  {
    regex: makeRegex("kilometres?|kilometers?|km?"),
    outputUnit: "miles",
    factor: 1 / kmPerMile,
    space: 1,
  }, // 'miles' always preceded by space
  {
    regex: makeRegex("metres?|meters?|m"),
    outputUnit: "'",
    factor: 1 / mPerFoot,
    space: 0,
  }, // foot sign is never spaces

  // temperature
  {
    regex: makeRegex("°?F|(?:degrees? )?fahrenheit"),
    outputUnit: "°C",
    factor: 1,
  },
  {
    regex: makeRegex("°?C|(?:degrees? )?celsius|(?:degrees? )?centigrade"),
    outputUnit: "°F",
    factor: 1,
  },
];

function convert(input: string): string | null {
  for (let { regex, outputUnit, factor, space } of conversions) {
    ...
    let resultNumber = Number.parseFloat(numberPart) * factor;
    ...
    // put it all together
    const converted =
      formatOutput(resultNumber, places(numberPart)) + spacePart + outputUnit;
